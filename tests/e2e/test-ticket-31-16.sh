...
setup() {
    ...
    TEST_DIR=$(mktemp -d -t claude-profiles-e2e-31-16.XXXXXX)
...
test_ticket_31_sync_setup_url_flag_noninteractive() {
    ...
    print_test "#31 - init --no-sync creates .claude-profiles dir"
    run_claude_profiles "$TICKET_M1" init --no-sync > /dev/null 2>&1
    assert_dir_exists "$TICKET_M1/.claude/.claude-profiles"

    # Step 2: verify no git remote configured
    print_test "#31 - no git remote after init --no-sync"
    local remotes
    # Suppress stderr: a non-git dir (the correct state after --no-sync) emits
    # "fatal: not a git repository" which must not be mistaken for a remote.
    remotes=$(git -C "$TICKET_M1/.claude/.claude-profiles" remote 2>/dev/null || true)
    if [ -z "$remotes" ]; then
    ...
    output=$(run_claude_profiles "$TICKET_M1" sync setup --url "$TICKET_REMOTE" 2>&1)
    exit_code=$?
    ...
    local remote_url
    remote_url=$(git -C "$TICKET_M1/.claude/.claude-profiles" remote get-url origin 2>&1)
...
test_ticket_16_statusline_sync() {
    ...
    run_claude_profiles "$TICKET_M1" init --sync --url "$TICKET_REMOTE" > /dev/null 2>&1
    assert_dir_exists "$TICKET_M1/.claude/.claude-profiles"

    print_test "#16 - init machine 2 with sync"
    run_claude_profiles "$TICKET_M2" init --sync --url "$TICKET_REMOTE" > /dev/null 2>&1
    assert_dir_exists "$TICKET_M2/.claude/.claude-profiles"
...
STATUSLINE
    ...
    run_claude_profiles "$TICKET_M1" sync push > /dev/null 2>&1

    # Step 4: verify statusline.sh is in the claude-profiles sync repo
    assert_file_exists "$TICKET_M1/.claude/.claude-profiles/statusline.sh"
    assert_file_contains "$TICKET_M1/.claude/.claude-profiles/statusline.sh" "Custom statusline script"

    # Step 5: pull on machine 2
    print_test "#16 - pull statusline.sh on machine 2"
    run_claude_profiles "$TICKET_M2" sync pull --force > /dev/null 2>&1
